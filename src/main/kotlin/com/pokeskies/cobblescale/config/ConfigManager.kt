...
object ConfigManager {
    private var assetPackage = "assets/${CobbleScale.MOD_ID}"

    lateinit var CONFIG: CobbleScaleConfig
    ...
    fun load() {
        ...
        // Load all files
        CONFIG = loadFile("config.json", CobbleScaleConfig())
    }

    private fun copyDefaults() {
        val classLoader = CobbleScale::class.java.classLoader

        CobbleScale.INSTANCE.configDir.mkdirs()

        attemptDefaultFileCopy(classLoader, "config.json")
    }

    fun <T : Any> loadFile(filename: String, default: T, create: Boolean = false): T {
        val file = File(CobbleScale.INSTANCE.configDir, filename)
        var value: T = default
        try {
            Files.createDirectories(CobbleScale.INSTANCE.configDir.toPath())
            if (file.exists()) {
                FileReader(file).use { reader ->
                    val jsonReader = JsonReader(reader)
                    value = CobbleScale.INSTANCE.gsonPretty.fromJson(jsonReader, default::class.java)
                }
            } else if (create) {
                Files.createFile(file.toPath())
                FileWriter(file).use { fileWriter ->
                    fileWriter.write(CobbleScale.INSTANCE.gsonPretty.toJson(default))
                    fileWriter.flush()
                }
            }
        } catch (e: IOException) {
    ...
    fun <T> saveFile(filename: String, `object`: T): Boolean {
        val dir = CobbleScale.INSTANCE.configDir
        val file = File(dir, filename)
        try {
            FileWriter(file).use { fileWriter ->
                fileWriter.write(CobbleScale.INSTANCE.gsonPretty.toJson(`object`))
                fileWriter.flush()
            }
        } catch (e: Exception) {
            e.printStackTrace()
    ...
    private fun attemptDefaultFileCopy(classLoader: ClassLoader, fileName: String) {
        val file = CobbleScale.INSTANCE.configDir.resolve(fileName)
        if (!file.exists()) {
            try {
    ...
    private fun attemptDefaultDirectoryCopy(classLoader: ClassLoader, directoryName: String) {
        val directory = CobbleScale.INSTANCE.configDir.resolve(directoryName)
        if (!directory.exists()) {
            directory.mkdirs()
            try {
